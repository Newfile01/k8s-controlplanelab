...
const finalizerName = "controlplanetest.lab.local/finalizer"
...
func (r *ControlPlaneTestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Vérification de la présence d'un finalizer : "ressource en cours de suppression ?"
	if controlPlaneTest.ObjectMeta.DeletionTimestamp.IsZero() {
		// Finalizer absent → ajout
		if !controllerutil.ContainsFinalizer(controlPlaneTest, finalizerName) {
			// Ressource pas encore marquée
			fmt.Println("Ajout du Finalizer ...")
		...
		// La ressource est en suppression
		fmt.Println("Suppression détectée ...")
		if controllerutil.ContainsFinalizer(controlPlaneTest, finalizerName) {
			fmt.Println("Nettoyage avant suppression ...")
		...
		fmt.Println("Finalizer supprimé")
